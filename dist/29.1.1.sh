...
# We do not use the upstream start_docker_daemon, we use start_docker defined above
# start_docker_daemon() { ... }
...
do_install() {
    ...
    if command_exists docker; then
        ...
        echo ""
        echo "If you already have Docker installed, this script can cause trouble, which is"
        echo "why we're displaying this warning and provide the opportunity to cancel the"
        echo "installation."
        echo ""
        echo "If you installed the current Docker package using this script and are using it"
        echo "again to update Docker, you can ignore this message, but be aware that the"
        echo "script resets any custom changes in the deb and rpm repo configuration"
        echo "files to match the parameters passed to the script."
        echo ""
    ...
    case "$lsb_dist" in
        ubuntu|debian|raspbian)
            ...
            # Use start_docker instead of start_docker_daemon
        ...
        sles)
           ...
           (
               ...
               if ! zypper lr -d | grep -q "${opensuse_factory_url}"; then
                   ...
                   if ! is_dry_run; then
                       echo "WARNING!!"
                       echo "openSUSE repository ($opensuse_repo) will be enabled now."
           ...
           if ! command_exists iptables; then
               $sh_c "zypper install -y -q iptables"
           fi
            start_docker
            )
           echo_docker_as_nonroot
